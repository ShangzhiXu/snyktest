...
static int x509_crt_check_parent(const mbedtls_x509_crt *child,
  ...
  if (top && bottom && child->raw.len == parent->raw.len &&
...
static int x509_crt_verify_child(
    ...
    const mbedtls_x509_crt_profile *profile, int path_cnt, int self_cnt,
    uint32_t *flags, int (*f_vrfy)(void *, mbedtls_x509_crt *, int, uint32_t *),
    void *p_vrfy) {
  int ret;
  ...
  *flags |= x509_crt_verifycrl(child, parent, ca_crl, profile);
  ...
  for (grandparent = trust_ca; grandparent != NULL;
  ...
  if (grandparent != NULL) {
    ret =
        x509_crt_verify_top(parent, grandparent, ca_crl, profile, path_cnt + 1,
                            self_cnt, &parent_flags, f_vrfy, p_vrfy);
    if (ret != 0)
      return (ret);
  } else {
    /* Look for a grandparent upwards the chain */
    for (grandparent = parent->next; grandparent != NULL;
         grandparent = grandparent->next) {
    ...
    if (grandparent != NULL) {
      ret = x509_crt_verify_child(parent, grandparent, trust_ca, ca_crl,
                                  profile, path_cnt + 1, self_cnt,
                                  &parent_flags, f_vrfy, p_vrfy);
      if (ret != 0)
        return (ret);
    } else {
      ret = x509_crt_verify_top(parent, trust_ca, ca_crl, profile, path_cnt + 1,
                                self_cnt, &parent_flags, f_vrfy, p_vrfy);
      if (ret != 0)
        return (ret);
    }
...
int mbedtls_x509_crt_verify(
  ...
  return (mbedtls_x509_crt_verify_with_profile(
      crt, trust_ca, ca_crl, &mbedtls_x509_crt_profile_default, cn, flags,
      f_vrfy, p_vrfy));
}
  ...
  if (cn != NULL) {
    ...
    if (crt->ext_types & MBEDTLS_X509_EXT_SUBJECT_ALT_NAME) {
      ...
      while (cur != NULL) {
        ...
        if (cur->buf.len > 2 && memcmp(cur->buf.p, "*.", 2) == 0 &&
      ...
      while (name != NULL) {
        if (MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &name->oid) == 0) {
          ...
          if (name->val.len > 2 && memcmp(name->val.p, "*.", 2) == 0 &&
