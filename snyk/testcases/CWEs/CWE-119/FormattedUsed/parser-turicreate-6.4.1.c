 ...
 * parser option.
 */
static int xmlParserEntityCheck(xmlParserCtxtPtr ctxt, size_t size,
...
static const char *xmlW3CPIs[] = {"xml-stylesheet", "xml-model", NULL};
    ...
    if (RAW == ';') {
      ...
      if (entity == NULL) {
        ...
        if ((entity->etype == XML_INTERNAL_PARAMETER_ENTITY) ||
           ...
           */
          GROW if (ctxt->instate == XML_PARSER_EOF) return;
...
void xmlParsePI(xmlParserCtxtPtr ctxt) {
  ...
  if ((RAW == '<') && (NXT(1) == '?')) {
    ...
    if (target != NULL) {
      ...
      if ((len > XML_MAX_TEXT_LENGTH) &&
          ((ctxt->options & XML_PARSE_HUGE) == 0)) {
        xmlFatalErrMsgStr(ctxt, XML_ERR_PI_NOT_FINISHED, "PI %s too big found",
                          target);
        xmlFree(buf);
        ctxt->instate = state;
        return;
  ...
  while (ctxt->instate != XML_PARSER_EOF) {
    ...
    if (ctxt->input->buf == NULL)
      ...
      if ((ctxt->instate != XML_PARSER_START) &&
        ...
        xmlBufSetInputBaseCur(ctxt->input->buf->buffer, ctxt->input, base,
                              current);
      }
    ...
    case XML_PARSER_DTD: {
      ...
      for (; (unsigned int)base < use; base++) {
        ...
        if ((quote == 0) && (buf[base] == '<')) {
          ...
          if (((unsigned int)base + 4 < use) && (buf[base + 1] == '!') &&
 ...
 */
static int xmlParseCheckTransition(xmlParserCtxtPtr ctxt, const char *chunk,
                                   int size) {
...
xmldecl_done:
  ...
  if (remain != 0) {
     ...
     */
    if ((terminate) || (avail > XML_MAX_TEXT_LENGTH) || (old_avail == 0) ||
        (avail == 0) ||
  ...
  if (terminate) {
    ...
    if (ctxt->input != NULL) {
      if (ctxt->input->buf == NULL)
        cur_avail =
            ctxt->input->length - (ctxt->input->cur - ctxt->input->base);
      else
