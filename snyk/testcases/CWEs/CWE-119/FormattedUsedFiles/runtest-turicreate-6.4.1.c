 ...
 */
static xmlParserInputPtr testExternalEntityLoader(const char *URL,
                                                  const char *ID,
...
static void XMLCDECL testErrorHandler(void *ctx ATTRIBUTE_UNUSED,
                                      const char *msg, ...) {
  va_list args;
  int res;
 ...
 * extra parameters.
 */
static void XMLCDECL fatalErrorDebug(void *ctx ATTRIBUTE_UNUSED,
                                     const char *msg, ...) {
  va_list args;
...
static int schemasOneTest(const char *sch, const char *filename,
                          const char *result, const char *err, int options,
  ...
  xmlSchemaSetValidErrors(ctxt, (xmlSchemaValidityErrorFunc)testErrorHandler,
                          (xmlSchemaValidityWarningFunc)testErrorHandler, ctxt);
...
static int schemasTest(const char *filename, const char *resul ATTRIBUTE_UNUSED,
  ...
  xmlSchemaSetParserErrors(ctxt, (xmlSchemaValidityErrorFunc)testErrorHandler,
  ...
  for (i = 0; i < globbuf.gl_pathc; i++) {
    ...
    if ((len > 6) && (base2[len - 6] == '_')) {
      count = base2[len - 5];
      snprintf(result, 499, "result/schemas/%s_%c", prefix, count);
      result[499] = 0;
      snprintf(err, 499, "result/schemas/%s_%c.err", prefix, count);
      err[499] = 0;
    } else {
    ...
    if (schemas == NULL) {
      ...
      nb_tests++;
      ret = schemasOneTest(filename, instance, result, err, options, schemas);
      if (ret != 0)
        res = ret;
    }
  }
 ...
 *									*
 ************************************************************************/
static int rngOneTest(const char *sch, const char *filename, const char *result,
                      const char *err, int options, xmlRelaxNGPtr schemas) {
  xmlDocPtr doc;
  ...
  xmlRelaxNGSetValidErrors(ctxt, (xmlRelaxNGValidityErrorFunc)testErrorHandler,
...
static int rngTest(const char *filename, const char *resul ATTRIBUTE_UNUSED,
  ...
  xmlRelaxNGSetParserErrors(ctxt, (xmlRelaxNGValidityErrorFunc)testErrorHandler,
                            (xmlRelaxNGValidityWarningFunc)testErrorHandler,
                            ctxt);
  ...
  for (i = 0; i < globbuf.gl_pathc; i++) {
    ...
    if (schemas == NULL) {
      ...
      nb_tests++;
      ret = rngOneTest(filename, instance, result, err, options, schemas);
      if (res != 0)
        ret = res;
    }
  }
...
static const char *testfiles[] = {
    "test/threads/abc.xml",     "test/threads/acb.xml", "test/threads/bac.xml",
    "test/threads/bca.xml",     "test/threads/cab.xml", "test/threads/cba.xml",
...
static int testThread(void) {
  ...
  for (repeat = 0; repeat < 500; repeat++) {
    ...
    for (i = 0; i < num_threads; i++) {
      tid[i] = spawn_thread(thread_specific_data, "xmlTestThread",
    ...
    for (i = 0; i < globbuf.gl_pathc; i++) {
      ...
      if ((result) && (!checkTestFile(result))) {
        ...
        if (res != 0) {
          ...
          if ((xmlMemUsed() != mem) && (extraMemoryFromResolver == 0)) {
