...
static const char *url_errs[] = {"success",
                                 "out of memory",
                                 "bad parameter",
                                 "unrecognized scheme",
...
static CURLcode ldap_connect(struct connectdata *conn, bool *done) {
  ...
  if (conn->handler->flags & PROTOPT_SSL)
    *ptr++ = 's';
  snprintf(ptr, sizeof(hosturl) - (ptr - hosturl), "://%s:%d", conn->host.name,
           conn->remote_port);
...
static CURLcode ldap_connecting(struct connectdata *conn, bool *done) {
  ...
  if (conn->handler->flags & PROTOPT_SSL) {
    ...
    if (!li->ssldone) {
      CURLcode result =
          Curl_ssl_connect_nonblocking(conn, FIRSTSOCKET, &li->ssldone);
...
static CURLcode ldap_do(struct connectdata *conn, bool *done) {
  ...
  rc = ldap_search_ext(li->ld, ludp->lud_dn, ludp->lud_scope, ludp->lud_filter,
                       ludp->lud_attrs, 0, NULL, NULL, NULL, 0, &msgid);
