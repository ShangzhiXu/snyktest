 ...
 *  MAC algorithm (5.2.3.1) uses only 32-bit length in place of
 * seq_num/type/length fields MAC algorithm uses SHA1 and MD5 for the two hash
 * functions instead of one or other key_block algorithm (6.2.2) uses 'X', 'YY',
 * 'ZZZ' instead of 'A', 'BB', 'CCC' key_block partitioning is different (16
 * bytes each: MAC secret, decrypt key, encrypt key) encryption/decryption keys
 * updated every 4096 packets See http://wp.netscape.com/eng/ssl3/draft302.txt
 */

/*
 * 48-byte transformation used to generate master secret (6.1) and key material
 * (6.2.2). Both SHA1 and MD5 algorithms are used.
 */
...
static void sec_generate_keys(RDPCLIENT *This, uint8 *client_random,
  ...
  sec_hash_48(master_secret, pre_master_secret, client_random, server_random,
              'A');
