...
SDL_Surface *SDL_LoadBMP_RW(SDL_RWops *src, int freesrc) {
  ...
  if (biSize == 12) { /* really old BITMAPCOREHEADER */
    ...
    /* 64 == BITMAPCOREHEADER2, an incompatible OS/2 2.x extension. Skip this
     * stuff for now. */
    if (biSize == 64) {
  ...
  surface = SDL_CreateRGBSurface(0, biWidth, biHeight, biBitCount, Rmask, Gmask,
                                 Bmask, Amask);
  if (surface == NULL) {
    was_error = SDL_TRUE;
    goto done;
  ...
  if (palette) {
    ...
    if ((int)biClrUsed > palette->ncolors) {
      ...
      colors = (SDL_Color *)SDL_realloc(palette->colors,
                                        ncolors * sizeof(*palette->colors));
      if (!colors) {
        SDL_OutOfMemory();
        was_error = SDL_TRUE;
        goto done;
...
int SDL_SaveBMP_RW(SDL_Surface *saveme, SDL_RWops *dst, int freedst) {
  ...
  /* The additional header members from the Win32 BITMAPV4HEADER struct (108
   * bytes in total) */
  Uint32 bV4RedMask = 0;
  ...
  if (save32bit) {
    saveLegacyBMP =
        SDL_GetHintBoolean(SDL_HINT_BMP_SAVE_LEGACY_FORMAT, SDL_FALSE);
  }
