...
CURLcode Curl_fillreadbuffer(struct connectdata *conn, int bytes, int *nreadp) {
  ...
  if (!data->req.forbidchunk && data->req.upload_chunky) {
    ...
    hexlen =
        snprintf(hexbuffer, sizeof(hexbuffer), "%x%s", nread, endofline_native);
    ...
    *didwhat |= KEEP_RECV;
    ...
    if (k->str && !k->header && (nread > 0 || is_empty_data)) {
      ...
      if (k->chunk) {
        ...
        CHUNKcode res = Curl_httpchunk_read(conn, k->str, nread, &nread);
        ...
        if (excess > 0 && !k->ignorebody) {
          if (Curl_pipeline_wanted(conn->data->multi, CURLPIPE_HTTP1)) {
            infof(data,
                  ...
                  excess, data->state.path, k->size, k->maxdownload,
                  k->bytecount, nread);
        ...
        if (k->badheader && !k->ignorebody) {
          ...
          if (k->maxdownload == -1 || (curl_off_t)k->hbuflen <= k->maxdownload)
            result = Curl_client_write(conn, CLIENTWRITE_BODY,
                                       data->state.headerbuff, k->hbuflen);
          else
            result = Curl_client_write(conn, CLIENTWRITE_BODY,
...
static CURLcode readwrite_upload(struct Curl_easy *data,
  ...
  *didwhat |= KEEP_SEND;
  ...
  do {
    ...
    result = Curl_write(conn, conn->writesockfd, /* socket to send to */
...
CURLcode Curl_readwrite(struct connectdata *conn, struct Curl_easy *data,
  ...
  if (k->keepon) {
    if (0 > Curl_timeleft(data, &k->now, FALSE)) {
      if (k->size != -1) {
        failf(data,
              "Operation timed out after %ld milliseconds with "
              "%" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T
              " bytes received",
              Curl_timediff(k->now, data->progress.t_startsingle), k->bytecount,
              k->size);
      } else {
        failf(data,
              "Operation timed out after %ld milliseconds with "
  ...
  *done = (0 == (k->keepon &
                 (KEEP_RECV | KEEP_SEND | KEEP_RECV_PAUSE | KEEP_SEND_PAUSE)))
              ? TRUE
              : FALSE;
