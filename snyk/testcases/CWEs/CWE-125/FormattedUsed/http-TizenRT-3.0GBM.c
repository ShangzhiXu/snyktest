...
CURLcode Curl_http_auth_act(struct connectdata *conn) {
  ...
  if (pickhost || pickproxy) {
    ...
    if ((data->set.httpreq != HTTPREQ_GET) &&
        (data->set.httpreq != HTTPREQ_HEAD) && !conn->bits.rewindaftersend) {
...
static CURLcode output_auth_headers(struct connectdata *conn,
  ...
  if ((authstatus->picked == CURLAUTH_NEGOTIATE) && negdata->context &&
 ...
 */
CURLcode
Curl_http_output_auth(struct connectdata *conn, const char *request,
                      const char *path,
                                           ...
                                           up the proxy tunnel */
{
  ...
  if (!data->state.this_is_a_follow || conn->bits.netrc ||
      !data->state.first_host || data->set.allow_auth_to_other_hosts ||
...
static size_t readmoredata(char *buffer, size_t size, size_t nitems,
  ...
  conn->data->req.forbidchunk =
      (http->sending == HTTPSEND_REQUEST) ? TRUE : FALSE;
...
CURLcode Curl_http_done(struct connectdata *conn, CURLcode status,
                        bool premature) {
  struct Curl_easy *data = conn->data;
  struct HTTP *http = data->req.protop;
  ...
  if (!premature && /* this check is pointless when DONE is called before the
      ...
      (http->readbytecount + data->req.headerbytecount -
...
CURLcode Curl_add_custom_headers(struct connectdata *conn, bool is_connect,
                                 Curl_send_buffer *req_buffer) {
  char *ptr;
...
#define TE_HEADER "TE: gzip\r\n"
  ...
  if (http->sendit) {
    ...
    result =
        Curl_mime_prepare_headers(http->sendit, cthdr, NULL, MIMESTRATEGY_FORM);
  ...
  if (paste_ftp_userpwd)
    result = Curl_add_bufferf(req_buffer, "ftp://%s:%s@%s", conn->user,
                              conn->passwd, ppath + sizeof("ftp://") - 1);
  else
  ...
  case HTTPREQ_PUT: /* Let's PUT the data to the server! */
    ...
    result = Curl_add_buffer_send(req_buffer, conn, &data->info.request_size, 0,
                                  FIRSTSOCKET);
    if (result)
  ...
  do {
    ...
    *nread -= (ssize_t)rest_length;
    ...
    if ((0x0a == *k->p) || (0x0d == *k->p)) {
      ...
      result =
          Curl_client_write(conn, writetype, data->state.headerbuff, headerlen);
      if (result)
        return result;
...
#define HEADER1 k->p /* no conversion needed, just use k->p */
      ...
      if (conn->handler->protocol & PROTO_FAMILY_HTTP) {
         ...
         */
        char separator;
        nc = sscanf(HEADER1, " HTTP/%1d.%1d%c%3d", &httpversion_major,
                    &conn->httpversion, &separator, &k->httpcode);
        ...
        char separator;
        nc = sscanf(HEADER1, " RTSP/%1d.%1d%c%3d", &rtspversion_major,
                    &conn->rtspversion, &separator, &k->httpcode);
        if ((nc == 4) && (' ' == separator)) {
      ...
      */
      ...
      if (result)
        return result;
               ...
               checkprefix("Location:", k->p) && !data->req.location) {
