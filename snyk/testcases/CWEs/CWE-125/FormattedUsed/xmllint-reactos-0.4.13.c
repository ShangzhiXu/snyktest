...
static void myFreeFunc(void *mem) { xmlMemFree(mem); }
...
static int my_gettimeofday(struct timeval *tvp, void *tzp) {
 ...
 */
static void startTimer(void) { gettimeofday(&begin, NULL); }
 ...
 *           type argument
 */
static void XMLCDECL LIBXML_ATTR_FORMAT(1, 2) endTimer(const char *fmt, ...) {
  long msec;
  va_list ap;
...
static void startTimer(void) { begin = clock(); }
static void XMLCDECL LIBXML_ATTR_FORMAT(1, 2) endTimer(const char *fmt, ...) {
  long msec;
  va_list ap;
...
static void XMLCDECL LIBXML_ATTR_FORMAT(1, 2) endTimer(char *format, ...) {
  /*
...
static void xmlHTMLPrintFileContext(xmlParserInputPtr input) {
  ...
  while ((*cur != 0) && (*cur != '\n') && (*cur != '\r') && (n < 79)) {
    len = strlen(buffer);
    snprintf(&buffer[len], sizeof(buffer) - len, "%c", (unsigned char)*cur++);
    n++;
  }
  len = strlen(buffer);
  snprintf(&buffer[len], sizeof(buffer) - len, "\n");
 ...
 */
static char *xmlShellReadline(char *prompt) {
#ifdef HAVE_LIBREADLINE
  char *line_read;
 ...
 */
static void entityDeclDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *name,
...
static void processNode(xmlTextReaderPtr reader) {
  ...
  if (debug) {
    ...
    printf("%d %d %s %d %d", xmlTextReaderDepth(reader), type, name, empty,
...
static void streamFile(char *filename) {
  ...
  if (memory) {
    ...
    reader = xmlReaderForMemory(base, info.st_size, filename, NULL, options);
  } else
#endif
...
static void walkDoc(xmlDocPtr doc) {
  ...
  if (pattern != NULL) {
    patternc = xmlPatterncompile((const xmlChar *)pattern, doc->dict, 0,
                                 &namespaces[0]);
    if (patternc == NULL) {
      xmlGenericError(xmlGenericErrorContext, "Pattern %s failed to compile\n",
                      pattern);
...
static void parseAndPrintFile(char *filename, xmlParserCtxtPtr rectxt) {
  ...
  if (filename == NULL) {
    ...
    if (f != NULL) {
      ...
      if (res > 0) {
        ctxt = htmlCreatePushParserCtxt(NULL, NULL, chars, res, filename,
    ...
    doc = htmlReadMemory((char *)base, info.st_size, filename, NULL, options);
    ...
    if (push) {
      ...
      if ((filename[0] == '-') && (filename[1] == 0)) {
        ...
        if (f != NULL) {
          if (rectxt == NULL)
            doc = xmlReadIO(myRead, myClose, f, filename, NULL, options);
          else
            doc = xmlCtxtReadIO(rectxt, myRead, myClose, f, filename, NULL,
                                options);
        } else
          doc = NULL;
      }
      ...
      if (rectxt == NULL)
        doc =
            xmlReadMemory((char *)base, info.st_size, filename, NULL, options);
      else
        doc = xmlCtxtReadMemory(rectxt, (char *)base, info.st_size, filename,
                                NULL, options);
  ...
  if (noout == 0) {
    ...
    if (!debug) {
      ...
      if ((html) && (!xmlout)) {
        ...
        size = xmlC14NDocDumpMemory(doc, NULL, XML_C14N_EXCLUSIVE_1_0, NULL, 1,
                                    &result);
        if (size >= 0) {
          if (write(1, result, size) == -1) {
            fprintf(stderr, "Can't write data\n");
        ...
        if (encoding != NULL) {
          if (format == 1) {
            ret = xmlSaveFormatFileEnc(output ? output : "-", doc, encoding, 1);
          } else {
...
static void usage(FILE *f, const char *name) {
...
#ifdef LIBXML_VALID_ENABLED
  fprintf(f, "\t--valid : validate the document in addition to std well-formed "
             "check\n");
  fprintf(f, "\t--postvalid : do a posteriori validation, i.e after parsing\n");
  fprintf(
      f, "\t--dtdvalid URL : do a posteriori validation against a given DTD\n");
  fprintf(
      f,
  ...
  fprintf(f, "\t--pushsmall : use the push mode of the parser using tiny "
             "increments\n");
#endif /* LIBXML_PUSH_ENABLED */
#ifdef HAVE_MMAP
  ...
  fprintf(f, "\t--exc-c14n : save in W3C exclusive canonical format (with "
             "comments)\n");
#ifdef LIBXML_C14N_ENABLED
...
#ifdef LIBXML_DEBUG_ENABLED
        if ((!strcmp(argv[i], "-shell")) || (!strcmp(argv[i], "--shell"))) {
      shell++;
      noout = 1;
    } else
#endif
