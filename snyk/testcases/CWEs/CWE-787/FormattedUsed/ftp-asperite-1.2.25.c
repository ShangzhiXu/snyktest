...
static CURLcode tftp_parse_option_ack(tftp_state_data_t *state, const char *ptr,
                                      int len) {
  const char *tmp = ptr;
...
static size_t tftp_option_add(tftp_state_data_t *state, size_t csize, char *buf,
                              const char *option) {
...
static CURLcode tftp_send_first(tftp_state_data_t *state, tftp_event_t event) {
  ...
  case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
    ...
    /* As RFC3617 describes the separator slash is not actually part of the
       ...
       string. */
    filename =
        curl_easy_unescape(data, &state->conn->data->state.path[1], 0, NULL);
    if (!filename)
      return CURLE_OUT_OF_MEMORY;

    snprintf((char *)state->spacket.data + 2, state->blksize, "%s%c%s%c",
             filename, '\0', mode, '\0');
    ...
    senddata = sendto(state->sockfd, (void *)state->spacket.data,
                      (SEND_TYPE_ARG3)sbytes, 0, state->conn->ip_addr->ai_addr,
...
static CURLcode tftp_rx(tftp_state_data_t *state, tftp_event_t event) {
  ...
  case TFTP_EVENT_DATA:
    ...
    if (NEXT_BLOCKNUM(state->block) != rblock) {
      ...
      infof(data, "Received unexpected DATA packet block %d\n", rblock);
      state->retries++;
      if (state->retries > state->retry_max) {
