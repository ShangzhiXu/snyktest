...
BOOL WINAPI WVTAsn1SpcLinkEncode(DWORD dwCertEncodingType,
                                 LPCSTR lpszStructType,
                                 const void *pvStructInfo, BYTE *pbEncoded,
                                 DWORD *pcbEncoded) {
  BOOL ret = FALSE;
  ...
  __TRY {
    ...
    case SPC_FILE_LINK_CHOICE: {
      ...
      fileNameLen =
          link->u.pwszFile ? lstrlenW(link->u.pwszFile) * sizeof(WCHAR) : 0;
      CRYPT_EncodeLen(fileNameLen, NULL, &fileNameLenBytes);
      CRYPT_EncodeLen(1 + fileNameLenBytes + fileNameLen, NULL, &lenBytes);
    ...
    case SPC_MONIKER_LINK_CHOICE: {
      ...
      CRYPT_EncodeLen(link->u.Moniker.SerializedData.cbData, NULL,
                      &dataLenBytes);
      dataLen = 2 + classIdLenBytes + classId.cbData + dataLenBytes +
      ...
      if (!pbEncoded) {
        ...
        *pbEncoded++ = ASN_CONSTRUCTOR | ASN_CONTEXT | 1;
        ...
        CRYPT_AsnEncodeOctets(X509_ASN_ENCODING, NULL, &classId, pbEncoded,
                              &size);
        pbEncoded += size;
        size = 1 + dataLenBytes + link->u.Moniker.SerializedData.cbData;
        CRYPT_AsnEncodeOctets(X509_ASN_ENCODING, NULL,
                              &link->u.Moniker.SerializedData, pbEncoded,
                              &size);
        pbEncoded += size;
        ret = TRUE;
      }
  ...
  __EXCEPT_PAGE_FAULT { SetLastError(STATUS_ACCESS_VIOLATION); }
  __ENDTRY
  TRACE("returning %d\n", ret);
  return ret;
}

typedef BOOL(WINAPI *CryptEncodeObjectFunc)(DWORD, LPCSTR, const void *, BYTE *,
                                            DWORD *);
...
static BOOL CRYPT_AsnEncodeSequence(DWORD dwCertEncodingType,
  ...
  for (i = 0, ret = TRUE; ret && i < cItem; i++) {
    ret = items[i].encodeFunc(dwCertEncodingType, NULL, items[i].pvStructInfo,
                              NULL, &items[i].size);
    ...
    if (!pbEncoded)
      ...
      *pbEncoded++ = ASN_SEQUENCE;
      ...
      for (i = 0; ret && i < cItem; i++) {
        ret =
            items[i].encodeFunc(dwCertEncodingType, NULL, items[i].pvStructInfo,
...
static BOOL WINAPI CRYPT_AsnEncodeBits(DWORD dwCertEncodingType,
  ...
  __TRY {
    ...
    if (!blob->cUnusedBits) {
      ...
      unusedBits =
          blob->cUnusedBits >= 8 ? blob->cUnusedBits / 8 : blob->cUnusedBits;
    } else {
      dataBytes = 0;
      unusedBits = 0;
    }
...
BOOL WINAPI WVTAsn1SpcPeImageDataEncode(DWORD dwCertEncodingType,
  ...
  __TRY {
    ...
    ret = CRYPT_AsnEncodeSequence(dwCertEncodingType, items, cItem, pbEncoded,
                                  pcbEncoded);
  }
  __EXCEPT_PAGE_FAULT { SetLastError(STATUS_ACCESS_VIOLATION); }
  __ENDTRY
...
static BOOL WINAPI CRYPT_AsnEncodeAlgorithmIdWithNullParams(
  ...
  static const CRYPT_DATA_BLOB nullBlob = {sizeof(asn1Null), (LPBYTE)asn1Null};
  BOOL ret;
  ...
  ret = CRYPT_AsnEncodeSequence(dwCertEncodingType, items,
                                sizeof(items) / sizeof(items[0]), pbEncoded,
                                pcbEncoded);
  return ret;
}
...
static BOOL WINAPI CRYPT_AsnEncodeSwapTag(DWORD dwCertEncodingType,
  ...
  ret = item->encodeFunc(dwCertEncodingType, lpszStructType, item->pvStructInfo,
...
BOOL WINAPI WVTAsn1SpcSpOpusInfoEncode(DWORD dwCertEncodingType,
  ...
  __TRY {
    ...
    if (info->pMoreInfo &&
      ...
      ret = CRYPT_AsnEncodeSequence(X509_ASN_ENCODING, items, cItem, pbEncoded,
                                    pcbEncoded);
    }
  }
  __EXCEPT_PAGE_FAULT { SetLastError(STATUS_ACCESS_VIOLATION); }
  ...
  __TRY {
    ...
    if (significantBytes) {
      if (blob->pbData[significantBytes - 1] & 0x80) {
        ...
        for (;
             significantBytes > 0 && blob->pbData[significantBytes - 1] == 0xff;
             significantBytes--)
          ;
        if (blob->pbData[significantBytes - 1] < 0x80) {
          padByte = 0xff;
    ...
    if (!pvStructInfo)
      ...
      *pcbStructInfo = bytesNeeded;
      ...
      if (dwFlags & CRYPT_DECODE_NOCOPY_FLAG)
        ...
        assert(blob->pbData);
        if (blob->cbData)
          memcpy(blob->pbData, pbEncoded + 1 + lenBytes, blob->cbData);
      }
    }
  }
  return ret;
...
static BOOL CRYPT_AsnDecodeSPCLinkInternal(
  ...
  if ((ret = CRYPT_GetLen(pbEncoded, cbEncoded, &dataLen))) {
    ...
    case ASN_CONTEXT:
      ...
      if (!pvStructInfo)
        ...
        *pcbStructInfo = bytesNeeded;
        ...
        for (i = 0; i < dataLen; i++)
          link->u.pwszUrl[i] = *(pbEncoded + 1 + lenBytes + i);
        link->u.pwszUrl[i] = '\0';
    ...
    case ASN_CONSTRUCTOR | ASN_CONTEXT | 1: {
      ...
      if ((ret = CRYPT_AsnDecodeOctets(
               dwCertEncodingType, NULL, pbEncoded + 1 + lenBytes,
               cbEncoded - 1 - lenBytes, CRYPT_DECODE_NOCOPY_FLAG, &classId,
               &size))) {
          ...
          if ((ret = CRYPT_AsnDecodeOctets(
            ...
            if (!pvStructInfo)
              ...
              *pcbStructInfo = bytesNeeded;
               ...
               */
              link->u.Moniker.SerializedData.pbData = (BYTE *)link->u.pwszFile;
              memcpy(link->u.Moniker.ClassId, classId.pbData, classId.cbData);
              memcpy(link->u.Moniker.SerializedData.pbData, data.pbData,
                     data.cbData);
...
BOOL WINAPI WVTAsn1SpcLinkDecode(DWORD dwCertEncodingType,
  ...
  __TRY {
    ...
    ret = CRYPT_AsnDecodeSPCLinkInternal(dwCertEncodingType, lpszStructType,
                                         pbEncoded, cbEncoded, dwFlags, NULL,
                                         &bytesNeeded);
    if (ret) {
      if (!pvStructInfo)
        *pcbStructInfo = bytesNeeded;
        ...
        *pcbStructInfo = bytesNeeded;
        ...
        link->u.pwszFile = (LPWSTR)((BYTE *)pvStructInfo + sizeof(SPC_LINK));
        ret = CRYPT_AsnDecodeSPCLinkInternal(dwCertEncodingType, lpszStructType,
...
typedef BOOL(WINAPI *CryptDecodeObjectFunc)(DWORD, LPCSTR, const BYTE *, DWORD,
                                            DWORD, void *, DWORD *);
...
#define ALIGN_DWORD_PTR(x)                                                     \
  (((x) + sizeof(DWORD_PTR) - 1) & ~(sizeof(DWORD_PTR) - 1))
...
static BOOL CRYPT_AsnDecodeSequenceItems(DWORD dwCertEncodingType,
                                         ...
                                         DWORD cItem, const BYTE *pbEncoded,
                                         DWORD cbEncoded, DWORD dwFlags,
                                         void *pvStructInfo, BYTE *nextData,
                                         DWORD *cbDecoded) {
  BOOL ret;
  DWORD i, decoded = 0;
      ...
      if ((ret = CRYPT_GetLen(ptr, cbEncoded - (ptr - pbEncoded),
        ...
        if (ptr[0] == items[i].tag || !items[i].tag) {
          if (nextData && pvStructInfo && items[i].hasPointer) {
            ...
            *(BYTE **)((BYTE *)pvStructInfo + items[i].pointerOffset) =
                nextData;
          }
          if (items[i].decodeFunc) {
            if (pvStructInfo)
              ...
              TRACE("sizing item %d\n", i);
            ret = items[i].decodeFunc(
                dwCertEncodingType, NULL, ptr,
                1 + nextItemLenBytes + nextItemLen,
                dwFlags & ~CRYPT_DECODE_ALLOC_FLAG,
                pvStructInfo ? (BYTE *)pvStructInfo + items[i].offset : NULL,
                &items[i].size);
            if (ret) {
  ...
  if (pbEncoded[0] == ASN_SEQUENCE) {
    ...
    if ((ret = CRYPT_GetLen(pbEncoded, cbEncoded, &dataLen))) {
      ...
      if (cbEncoded < dataLen) {
        ...
        ret =
            CRYPT_AsnDecodeSequenceItems(dwFlags, items, cItem, ptr, cbEncoded,
                                         dwFlags, NULL, NULL, &cbDecoded);
        ...
        if (!pvStructInfo)
          ...
          *pcbStructInfo = bytesNeeded;
          ...
          ret = CRYPT_AsnDecodeSequenceItems(dwFlags, items, cItem, ptr,
                                             cbEncoded, dwFlags, pvStructInfo,
                                             nextData, &cbDecoded);
        }
      }
    }
...
static BOOL WINAPI CRYPT_AsnDecodeBitsInternal(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  BOOL ret;
...
static BOOL WINAPI CRYPT_AsnDecodeSPCLinkPointer(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  BOOL ret = FALSE;
  DWORD dataLen;
      ...
      if (!pvStructInfo)
        ...
        *pcbStructInfo = size + sizeof(PSPC_LINK);
        ...
        if (size > sizeof(SPC_LINK)) {
          (*pLink)->u.pwszUrl = (LPWSTR)((BYTE *)*pLink + sizeof(SPC_LINK));
        }
        ret = CRYPT_AsnDecodeSPCLinkInternal(dwCertEncodingType, lpszStructType,
                                             pbEncoded + 1 + lenBytes, dataLen,
                                             dwFlags, *pLink, pcbStructInfo);
      }
    }
  }
  return ret;
...
BOOL WINAPI WVTAsn1SpcPeImageDataDecode(DWORD dwCertEncodingType,
  ...
  __TRY {
    ...
    ret = CRYPT_AsnDecodeSequence(
        dwCertEncodingType, items, sizeof(items) / sizeof(items[0]), pbEncoded,
        cbEncoded, dwFlags, pvStructInfo, pcbStructInfo, NULL);
  }
  __EXCEPT_PAGE_FAULT { SetLastError(STATUS_ACCESS_VIOLATION); }
  __ENDTRY
  TRACE("returning %d\n", ret);
...
static BOOL WINAPI CRYPT_AsnDecodeOidIgnoreTag(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  BOOL ret = TRUE;
  DWORD dataLen;
  ...
  if ((ret = CRYPT_GetLen(pbEncoded, cbEncoded, &dataLen))) {
    ...
    if (dataLen) {
      ...
      sprintf(firstTwo, "%d.%d", pbEncoded[1 + lenBytes] / 40,
              pbEncoded[1 + lenBytes] - (pbEncoded[1 + lenBytes] / 40) * 40);
      bytesNeeded += strlen(firstTwo) + 1;
      for (ptr = pbEncoded + 2 + lenBytes;
           ret && ptr - pbEncoded - 1 - lenBytes < dataLen;) {
...
static BOOL WINAPI CRYPT_AsnDecodeCopyBytes(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  BOOL ret = TRUE;
...
static BOOL WINAPI CRYPT_AsnDecodeAttributeTypeValue(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  ...
  struct AsnDecodeSequenceItem items[] = {
      ...
      {0, offsetof(CRYPT_ATTRIBUTE_TYPE_VALUE, Value), CRYPT_AsnDecodeCopyBytes,
...
static BOOL WINAPI CRYPT_AsnDecodeAlgorithmId(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  ...
  if (ret && pvStructInfo) {
    TRACE("pszObjId is %p (%s)\n", algo->pszObjId, debugstr_a(algo->pszObjId));
  }
  return ret;
}

static BOOL WINAPI CRYPT_AsnDecodeSPCDigest(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  struct SPCDigest *digest = pvStructInfo;
  struct AsnDecodeSequenceItem items[] = {
      {ASN_SEQUENCEOF, offsetof(struct SPCDigest, DigestAlgorithm),
       CRYPT_AsnDecodeAlgorithmId, sizeof(CRYPT_ALGORITHM_IDENTIFIER), FALSE,
       TRUE, offsetof(struct SPCDigest, DigestAlgorithm.pszObjId), 0},
  ...
  __TRY {
    struct AsnDecodeSequenceItem items[] = {
        {ASN_SEQUENCEOF, offsetof(SPC_INDIRECT_DATA_CONTENT, Data),
         CRYPT_AsnDecodeAttributeTypeValue, sizeof(CRYPT_ATTRIBUTE_TYPE_VALUE),
...
static BOOL WINAPI CRYPT_AsnDecodeBMPString(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  BOOL ret;
  DWORD bytesNeeded, dataLen;
...
static BOOL WINAPI CRYPT_AsnDecodeProgramName(
    DWORD dwCertEncodingType, LPCSTR lpszStructType, const BYTE *pbEncoded,
    DWORD cbEncoded, DWORD dwFlags, void *pvStructInfo, DWORD *pcbStructInfo) {
  BOOL ret = FALSE;
  DWORD dataLen;
  ...
  TRACE("(%p, %d, %08x, %p, %d)\n", pbEncoded, cbEncoded, dwFlags, pvStructInfo,
        pvStructInfo ? *pcbStructInfo : 0);

  if ((ret = CRYPT_GetLen(pbEncoded, cbEncoded, &dataLen))) {
    BYTE lenBytes = GET_LEN_BYTES(pbEncoded[1]);

    ret = CRYPT_AsnDecodeBMPString(dwCertEncodingType, lpszStructType,
                                   pbEncoded + 1 + lenBytes, dataLen, dwFlags,
                                   pvStructInfo, pcbStructInfo);
  }
  return ret;
}
  ...
  __TRY {
    struct AsnDecodeSequenceItem items[] = {
        ...
        {ASN_CONSTRUCTOR | ASN_CONTEXT | 1,
         offsetof(SPC_SP_OPUS_INFO, pMoreInfo), CRYPT_AsnDecodeSPCLinkPointer,
         sizeof(PSPC_LINK), TRUE, TRUE, offsetof(SPC_SP_OPUS_INFO, pMoreInfo),
         0},
...
BOOL WINAPI WVTAsn1CatNameValueDecode(DWORD dwCertEncodingType,
  ...
  __TRY {
    struct AsnDecodeSequenceItem items[] = {
        ...
        {ASN_INTEGER, offsetof(CAT_NAMEVALUE, fdwFlags), CRYPT_AsnDecodeInt,
         MEMBERSIZE(CAT_NAMEVALUE, fdwFlags, Value), FALSE, FALSE, 0, 0},
        {ASN_OCTETSTRING, offsetof(CAT_NAMEVALUE, Value), CRYPT_AsnDecodeOctets,
         sizeof(CRYPT_DER_BLOB), FALSE, TRUE,
  ...
  __TRY {
    struct AsnDecodeSequenceItem items[] = {
        {ASN_BOOL, offsetof(SPC_FINANCIAL_CRITERIA, fFinancialInfoAvailable),
         CRYPT_AsnDecodeBool,
         MEMBERSIZE(SPC_FINANCIAL_CRITERIA, fFinancialInfoAvailable,
                    fMeetsCriteria),
         FALSE, FALSE, 0, 0},
        {ASN_BOOL, offsetof(SPC_FINANCIAL_CRITERIA, fMeetsCriteria),
         CRYPT_AsnDecodeBool,
         FINALMEMBERSIZE(SPC_FINANCIAL_CRITERIA, fMeetsCriteria), FALSE, FALSE,
         0, 0},
    };
